...
class Paddle{
	...
	float yAdd;	// sets how far the centre of the paddle should be from the court bottom
public:
	...
	void setDifficulty(Difficulty level){
		switch(level){
			...
			//set width of the paddle to be large
				width = 3.0f; make();currDifficulty = EASY; break;
			case MEDIUM:
				width = 2.0f; make();currDifficulty = MEDIUM; break;
			case HARD:
				width = 1.0f; make();currDifficulty = HARD; break;
		}
	}
	...
	//when called moves the paddle by changing its coordinates
	void move(){
		...
		if (currDirection == LEFT){
			if ((x_leftEdge - xStep) > courtXboundary[0]) xOffset -= xStep;
		}else if (currDirection == RIGHT){
			if ((x_rightEdge + xStep) <courtXboundary[1]) xOffset += xStep;
		}
	}
...
public:
	...
	void move(Paddle p){
		...
		if (xDirection == LEFT && (x_leftEdge+xStep) <= courtXboundary[0]){
			xStep = -xStep;
			xDirection = RIGHT;
			std::cout << "x direction now right\n";
		}else if(xDirection == RIGHT && (x_rightEdge+xStep) >= courtXboundary[1]){
			xStep = -xStep;
			xDirection = LEFT;
			std::cout << "x direction now keft\n";
		}
		if (yDirection == UP && (y_topEdge+yStep) >= courtYboundary[1]){
			yStep = -yStep;
			yDirection = DOWN;
		}else if (yDirection == DOWN && (y_bottomEdge-speed) <= paddleTopCoord){
		...
		if (yDirection == DOWN && y_bottomEdge-speed <= courtYboundary[0]){
...
class Block{
	...
	float width;		//stores the width of a block
	float height;		//stores the height of a block
...
class RowBlocks{
	...
	unsigned int basicBlock;		//will be used to execute a pre-compiled display list of a block to generate a row of blocks	
...
public:
	...
	RowBlocks(float courtWidth, unsigned int rowlvl, unsigned int nRows){
		numBlocks = 5;
		// print(courtWidth);
	...
	unsigned int getScore(){
		...
		return numHit * pointsPerBlock * (numRows - rowLevel);
	}
};
...
public:
	...
	void init(){
		...
		for (unsigned int i = 0; i < numRows; i++){
			...
			RowBlocks r(courtWidth,i,numRows);
			r.setColor(colors);
			r.init(gapHeight);
			// std::cout<<"one row\n";
			grid.push_back(r);
...
int lengthWord(char * s){
	int totallength = 0;
	for (int i = 0; i< strlen(s); i++)
		totallength += glutStrokeWidth(GLUT_STROKE_ROMAN,*(s + i));
	return totallength;
}
